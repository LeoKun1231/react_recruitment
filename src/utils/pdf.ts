 ...
 */
export async function outputPDF({ element, contentWidth = 592.28, footer, header, filename = '测试A4分页.pdf', isSave = true }: any) {
  ...
  function getElementTop(element: any) {
    let actualTop = element.offsetTop - 100
  ...
  function traversingNodes(nodes: any[]) {
    for (let i = 0; i < nodes.length; ++i) {
      ...
      // 特殊的富文本元素
      const isEditor = one.classList && one.classList.contains('rich-editor')
  ...
  function updateNomalElPos(top: number) {
  ...
  if (isSave) {
    ...
    const res = dataURLtoFile(pdf.output('datauristring'), filename)
    console.log(res)
    return res
  }
}
...
function dataURLtoFile(dataurl: string, filename: string) {
  ...
  const mime = arr[0].match(/:(.*?);/)[1] as any
  ...
  return new File([u8arr], filename, { type: mime }) //转换成成blob对象 //return new Blob([u8arr],{type:mime});
}
